...
var Client = /*#__PURE__*/function (_SubClient) {
  ...
  _createClass(Client, [{
    ...
    value: function connect(max, delay) {
      ...
      this.ws = new WebSocket__default["default"]('wss://broadcastlv.chat.bilibili.com:2245/sub');
            ...
            for (var offset = 0, _packetLen, _body; offset < data.byteLength; offset += _packetLen) {
              ...
              try {
                if (_ver === 2) {
                  ...
                  var bufBody = zlib__default["default"].inflateSync(buffer.Buffer.from(msgBody));
                  _body = _this2.convertToObject(bufBody.buffer).body;
                } else if (_ver === 3) {
                  var _msgBody = data.slice(offset + _headerLen, offset + _packetLen);

                  var _bufBody = zlib__default["default"].brotliDecompressSync(buffer.Buffer.from(_msgBody));
