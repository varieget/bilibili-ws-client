...
var Client = /*#__PURE__*/function (_SubClient) {
  function Client(token, logger, maxConnectTimes, delay) {
    var _this;
    ...
    _this.MAX_CONNECT_TIMES = maxConnectTimes !== null && maxConnectTimes !== void 0 ? maxConnectTimes : 6; // 最多重试次数
    _this.DELAY = delay !== null && delay !== void 0 ? delay : 3000; // 重试间隔
    ...
    if (typeof logger === 'function') {
      _this.log = logger.bind(_this);
    } else if (logger) {
      _this.log = console.log.bind(console);
    } else {
      _this.log = function () {};
    }
    _this.connect(_this.MAX_CONNECT_TIMES, _this.DELAY);
    return _this;
  }
  ...
  return _createClass(Client, [{
    ...
    value: function connect(max, delay) {
      ...
      var roomId = this.options.token;
      this.ws.onopen = function () {
        var _this2$ws;
        _this2.log('auth start');
        ...
        (_this2$ws = _this2.ws) === null || _this2$ws === void 0 || _this2$ws.send(_this2.convertToArrayBuffer(token, 7));
      };
        ...
        switch (op) {
          case 8:
            ...
            heartbeatInterval = setInterval(function () {
              var _this2$ws2;
              (_this2$ws2 = _this2.ws) === null || _this2$ws2 === void 0 || _this2$ws2.send(_this2.convertToArrayBuffer('', 2));
              _this2.log('send: heartbeat;');
            }, 30 * 1000);
            break;
          case 3:
            // 人气
            // heartbeat reply
            _this2.log('receive: heartbeat;', {
              online: body
            });
            ...
            for (var offset = 0, _packetLen, _body; offset < data.byteLength; offset += _packetLen) {
              ...
              try {
                if (_ver === 2) {
                  ...
                  var bufBody = zlib.inflateSync(buffer.Buffer.from(msgBody));
                  ...
                  var _bufBody = zlib.brotliDecompressSync(buffer.Buffer.from(_msgBody));
                ...
                _this2.log('messageReceived:', {
                  ver: _ver,
                  body: _body
                });
              } catch (e) {
                _this2.emit('error', e);
                _this2.log('decode body error:', e);
              }
            }
            break;
        }
      ...
      var reConnect = function reConnect() {
        if (--max === 0) {
          _this2.log('maxConnectTimes reached, reset delay.');
          max = _this2.MAX_CONNECT_TIMES - 1;
          delay = _this2.DELAY;
        }
        _this2.log('reConnect:', {
          max: max,
          delay: delay
        });
        _this2.connect(max, delay * 2);
      };
    }
  }, {
    key: "messageReceived",
    ...
    value: function close() {
      var _this$ws;
      (_this$ws = this.ws) === null || _this$ws === void 0 || _this$ws.close(1000);
    }
  }]);
}(SubClient);
